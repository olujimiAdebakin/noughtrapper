...
export default function ExamsTable({ clientName, staffName, examName }) {
  const router = useRouter();
  // const { staffName } = useParams();
  console.log("Props in ExamsTable:", { clientName, staffName, examName });
  const data = [
    {
      id: 1,
  ...
  const handleViewDetails = (examName) => {
    const encodedExamName = encodeURIComponent(
      examName.replace(/\s+/g, "-").toLowerCase()
    );
    const staff = staffName || "unknown";
    router.push(
      `/manage-clients/clients/${clientName}/${staffName}/${encodedExamName}`
    );
  };
                ...
                <div className="relative">
                  ...
                  {activeRow === item.id && (
                    <div className="absolute top-0 -right-7 mt-2 bg-white border rounded-lg shadow-lg p-1 min-w-20 z-40">
                      <div
                        onClick={() => handleViewDetails(item.examName)}
